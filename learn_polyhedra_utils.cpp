...
// void 表示这个函数没有返回值。
// PrintPolyhedra 是函数的名称。
// const shared_ptr<Shape>& shape 是这个函数的参数，表示一个指向 Shape 类型的智能指针。
// shared_ptr 是一种智能指针，负责自动管理对象的内存。
// 在这里，使用引用（&）避免了对智能指针本身的复制，并且 const 确保函数不会修改这个指针。
void PrintPolyhedra(const shared_ptr<Shape>& shape)
{
	// Polyhedra* A 声明一个名为 A 的指针，类型是 Polyhedra
	// static_cast<Polyhedra*>(shape.get()) 将原始指针转换为 Polyhedra 类型的指针。
	// shape.get() 获取 shared_ptr 所指向的原始指针（裸指针）。
	Polyhedra* A  = static_cast<Polyhedra*>(shape.get());
	// 调用 A 指针指向的 Polyhedra 对象的 GetPolyhedron 方法，并将返回的 Polyhedron 对象赋值给 PA。
...
// const shared_ptr<Shape>& cm1 是函数的第一个参数，表示一个指向 Shape 类型对象的智能指针
//使用引用（&）避免对智能指针本身的复制，并且 const 确保函数不会修改这个指针
void PrintPolyhedraActualPos(const shared_ptr<Shape>& cm1, const State& state1)
{
	// 从 state1 中获取 se3 成员，并将其绑定到常量引用 se3。
	const Se3r& se3 = state1.se3;
	// 获取 shared_ptr<Shape> 所指向的原始指针，并将其转换为指向 Polyhedra 类型的指针。
	Polyhedra*  A   = static_cast<Polyhedra*>(cm1.get());
	// 调用 Polyhedra 对象的 Initialize 成员函数
	A->Initialize();

	//move and rotate CGAL structure Polyhedron
	//从 se3.orientation 中获取旋转矩阵，并将其赋值给 rot_mat。
	Matrix3r       rot_mat   = (se3.orientation).toRotationMatrix();
	//从 se3.position 中获取平移向量，并将其赋值给 trans_vec
	...
	Transformation t_rot_trans(
	        ...
	        1);
	Polyhedron PA = A->GetPolyhedron();
	// 对 PA 中的所有点应用变换 t_rot_trans。返回points_begin() 中的Polyhedron顶点（覆盖原始数据）
